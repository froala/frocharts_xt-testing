...
generator = (function() {
	...
	gen = function() {
		...
		try {
			...
			this.otherAttributes = config && config.other_attributes;
		}
		catch(error){
	...
	gen.prototype = {
		...
		applyAttributes: function(chartType, baseData) {
			var generator = this,
				...
				otherAttributes = generator && generator.otherAttributes,
			...
			for(specKey in otherAttributes) {
				defaults = otherAttributes[specKey].defaults;
				for (attr in otherAttributes[specKey].key) {
					if(otherAttributes[specKey].key.hasOwnProperty(attr)) {
						keys = attr.split('-');
						console.log(attr);
						attrObj = otherAttributes[specKey].key[attr];
						attrNames = attrObj && attrObj.names;
						attrTypes = attrObj && attrObj.type;
						valueRange = attrObj && attrObj.range;
						...
						attrNames.forEach(function(attrName, index) {
							...
							values && values.forEach(function(val) {
								//console.log("\t\t"+val);
								cloneChartAttr = cloneBaseData = JSON.parse(JSON.stringify(baseData, null, 4));
								//cloneChartAttr = cloneBaseData && cloneBaseData.chart;
								//
								////Extend if require
								commonAttrib && generator.extend(cloneChartAttr, commonAttrib);
								...
								for(i = 0; i < keys.length; i++) {
									cloneChartAttr = cloneChartAttr && cloneChartAttr[keys[i]];
								}
								if (defaults) {
									cloneChartAttr && generator.extend(cloneChartAttr, defaults);
								}
								cloneChartAttr && (cloneChartAttr[attrName] = val.toString());
								//console.log("\t\t\t"+JSON.stringify(cloneBaseData));
								combinations.push({
									'spec_item_name': [specKey, attr, attrName, val].join(delim),
									'desc': "creating "+attr+" with "+ attrName+" as "+val+".",
									data: cloneBaseData
								});
							});
