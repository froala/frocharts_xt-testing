...
FusionCharts.register('module', ['private', 'modules.renderer.js-gantt', function () {
    ...
    chartapi('gantt', {
        ...
        series: function () {
            ...
            if (proLen) {
                ...
                for (i = 0; i < proLen; i += 1) {
                    ...
                    processIDMap[i] = id;
                    // Find the total process height defined in data
                    if (defined(processHeight = pluckNumber(processObj.height, chartDef.rowheight))) {
                        // Process height can't be negative
                        processHeight = mathAbs(processHeight);
                        totalHeight += processHeight || 0;
                        countDefinedHeight += 1;
                    }
                ...
                dataTable[headerId].countDefinedHeight = countDefinedHeight;
                dataTable[headerId].totalHeight = totalHeight;
                HCprocesses.height = dataTable[headerId].maxProcessHeight = maxHeight + 8;
        ...
        spaceManager: function (hc, definition, width, height) {
            ...
            if (process) {
                // Find the total number of processes which has pre-defined height
                countDefined = process.processCount - process.countDefinedHeight;
                ...
                newCanvasHeight = canvasHeight - process.totalHeight;
                // Find the available process height to draw the rest of the process
                // which doesn't have a pre-defined height.
                proAvH = newCanvasHeight / (countDefined ? countDefined : process.processCount);

                // Find if there is any empty space at the bottom to adjust
                if (!pluckNumber(chartDef.forcerowheight, 0) && !countDefined && process.totalHeight < canvasHeight) {
                ...
                if (maxRowH > proAvH) {
                    if (maxRowH - proAvH < 3) {
                        ...
                        processH = maxRowH =
                            newCanvasHeight / mathRound(newCanvasHeight / maxRowH);
                    }
                }
                process.maxProcessHeight = maxRowH;

                // Need a vertical scroll or not
                if (verScrl.enabled && (proAvH < maxRowH && countDefined) || proAvH < 0) {
                    processH = maxRowH;
                ...
                while (process) {
                    totalPH += process.dimension.h = forceRowHeight ? processH : (process.processHeight || processH);
                    if (nextRow = process.nextRow) {
                        while (nextRow) {
                            nextRow.dimension.h = process.dimension.h;
                            nextRow = nextRow.nextRow;
                        }
                    }
                    process = process.nextCol;
                }
                dataTable.processHeader.totalPH = totalPH;
                chart.processHeight = canvasHeight + dataTable.processHeader.dimension.h;
            }
        }
    ...
    renderer('renderer.gantt', {
        ...
        drawProcess: function (gridObj) {
            ...
            clickHandler = function (e) {
                 ...
                 * @param {boolean} isHeader - Specifies whether the event target is a process or process-header.
        ...
        drawCategories: function () {
            ...
            while (cat) {
                ...
                if (cat.isLast) {
                    gridBPath.push(M, xPos, yPos, v, totalPH - cat.dimension.y);
                    yPos -= gNTh * 0.5;
                    catH -= gNTh;
