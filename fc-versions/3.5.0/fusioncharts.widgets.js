    ...
    function () {
        ...
        global.core.addEventListener(['rendered', 'dataupdated', 'resized'], function (event) {
            ...
            chart.addEventListener('RealTimeUpdateComplete', function (event, logObj) {
                ...
                //
                //Also it has no meaning to add the menu when messages are not to be sent to log window
                //and instead need to be passed to a js handler
                //@todo: Need to check for exceptions.
                if (useMessageLog && !showRTMenuItem && messageGoesToLog &&
                    !alwaysShowMessageLogMenu && !messageLogger.menuCreated) {
...
FusionCharts.register('module', ['private', 'modules.renderer.js-realtime', function () {
    ...
    processAlerts = function (event) {
        ...
        var chartObj = event.sender,
            ...
            alerts = hcJSON && hcJSON.alerts && hcJSON.alerts.alert || [],
        ...
        checkBounds = function (value) {
            var alertObj,
                alertAction,
                val = numberFormatter.getCleanValue(value),
                j,
                exacAlertParamFN = function (alertObj) {
                    return function () {
                        /* jshint evil:true */
                        eval(alertObj.param);
                        /* jshint evil:false */
            ...
            for (j = 0; j < alertCount; j += 1) {
                ...
                if (alertObj.minvalue < val && alertObj.maxvalue > val) {
                    if (!(alertObj.occuronce === '1' && alertObj.hasOccurred)) {
                        ...
                        switch (alertAction) {
                            case ACTION_CALLJS:
                                setTimeout(exacAlertParamFN(alertObj), 0);
                                break;
        ...
        /*if (iapi.multiValueGauge) {
            ...
            dataCount = mathMax(values.length, ((iapi.hcInstance && iapi.hcInstance.options &&
            ...
            if (!state.lastSetValues || values[0] !== state.lastSetValues[0]) {
                checkBounds(Number(values[0]));
            }
        }
        */

        /*
         * The above code is commneted out as there is no need to differentiate between
         * a multi value gauge (which is Angular gauge or Linear gauge) and Multi series
         * real time chart. As both the types can have multiple values to be updated
         * through the real time API. Thus for both the cases it is best to loop through
         * all the values that are coming
         */

        // Get access to the length of the data (number of values).
        // No reference to the hcInstance object found as coded previously. Hence removed.
        dataCount = mathMax(values.length, 0);
...
FusionCharts.register('module', ['private', 'modules.renderer.js-widgets', function () {
    ...
    GaugeAxis.prototype = {
        ...
        calculateLimits: function (maxValue, minValue) {
            ...
            if (this.userMaxGiven && this.userMinGiven) {
                maxValue = userMax;
                minValue = userMin;
            }
            else {
    ...
    chartAPI('gaugebase', {
        ...
        chart: function (width, height) {
            var iapi = this,
                ...
                realtimeEnabled = iapi.realtimeEnabled,
            ...
            */
            ...
            //directly from message logger component on receipt of first log
                 ...
                 * @todo: Need to check for exceptions.
                 */
                if(HCChartObj.messageGoesToLog) {
                    (hc.callbacks ||
                        (hc.callbacks = [])).push(iapi.drawMLMenuButtons);
                }
            }

            if(realtimeEnabled && alertArr){
                iapi.hcJSON.alerts = iapi.parseAlertObj(alertArr);
            }
        ...
        /*
         * As realTimeExtension is defined later, we will create
         * a stub function which call teh parse alert object defined
         * in realTimeExtension
         */
        parseAlertObj: function(){
            return realTimeExtension.parseAlertObj.apply(this, arguments);
        },
    ...
    chartAPI('linearscalegauge', {
        ...
        spaceManager: function (hcJSON, fcJSON, width, height) {
            ...
            minCanW = mathMin( mathMax(canvasWidth * 0.3, 5), canvasWidth),
            minCanH = mathMin( mathMax(canvasHeight * 0.3, 5), canvasHeight),
            tickDimensions, captionWidth,
            rightLabel = 0, totalMargin;

            // For very low can-min size reduce the margin
            if (minCanW < 5 && width > 10) { // very low canvas height
                minCanW = canvasWidth = 5;
                totalMargin = chartRight + chartLeft;
                chartRight = HCChartObj.marginRight = (width - canvasWidth) * chartRight / totalMargin;
                chartLeft = HCChartObj.marginLeft = (width - canvasWidth) * chartLeft / totalMargin;
            }
            if (minCanH < 5 && height > 10) { // very low canvas height
                minCanH = canvasHeight = 5;
                totalMargin = chartTop + chartBottom;
                chartTop = HCChartObj.marginRight = (height - canvasHeight) * chartTop / totalMargin;
                chartBottom = HCChartObj.marginLeft = (height - canvasHeight) * chartBottom / totalMargin;
            }

            // Manage title Space
            if (this.manageTitleSpace && hcJSON.title.alignCaptionWithCanvas) {
                captionWidth = this.manageTitleSpace(hcJSON, fcJSON, minCanW, minCanH);
            }
        ...
        manageTitleSpace : function (hcJSON, fcJSON, minCanvasWidth, minCanvasHeight){
            ...
            canvasHeight = this.height - (HCChartObj.marginTop + HCChartObj.marginBottom),
            availableSpace = canvasHeight - minCanvasHeight,
            titleObj = iapi.titleSpaceManager(hcJSON, fcJSON, canvasWidth, availableSpace);
    ...
    chartAPI('thermometer', {
        ...
        spaceManager: function (hcJSON, fcJSON, width, height) {
            ...
            if (rDefined) {
                ...
                thmBulbExtraCurvatureHalf = (thmBulbRadius*2 - thmWidth)/2;
                if (xDefined) {
                    if (isOnLeft) {
                    ...
                    if (isOnLeft) {
                        // Adjust the left margin so that the widget is drawn in centre.
                        HCChartObj.marginLeft += ((width - chartLeft - chartRight - thmBulbExtraCurvatureHalf -
                            thmWidth)/2 - scaleWidth/2);

                        // Subtract the same value that you add to marginLeft here to bring widget to centre
                        HCChartObj.marginRight -= ((width - chartLeft - chartRight - thmBulbExtraCurvatureHalf -
                            thmWidth)/2 - scaleWidth/2);
    ...
    chartAPI('cylinder', {
        ...
        preSeriesAddition: function (HCObj, FCObj) {
            ...
            //Cylinder lighting
            HCChartObj.cyl3DLighting = pluckNumber(FCChartObj.use3dlighting, '1');

        },

        //return the point color as an object
        getPointColorObj: function (FCChartObj) {
            return {
                code: pluck(FCChartObj.gaugefillcolor, FCChartObj.thmfillcolor,
        ...
        spaceManager: function (hcJSON, fcJSON, width, height) {
            ...
            //Adjust marginLeft to centre align the widget horizontally
            HCChartObj.marginLeft += (canvasWidth / 2 - cylRadius);
    ...
    chartAPI('angulargauge', {
        ...
        series: function (FCObj, HCObj) {
            ...
            for (; index < length; index += 1) {
                ...
                baseRadius = pluckNumber(dialObj.baseradius, 0);
                ...
                series.data.push({
                    ...
                    baseRadius: pluckNumber(baseRadius * scaleFactor),
...
//                      gaugeBorderAlpha)) : undefined
            ...
            if (hasValidPoint) {
                ...
                for (index = 0; index < filteredData.length; index += 1) {
                    ...
                    hcDataObj = {
                        ...
                        style: lib.parsexAxisStyles(dataObj, {}, FCChartObj, dataLabelStyle, setColor),
                        categoryLabel: name,
                        toolText: toolText,
        ...
        spaceManager: function (hcJSON, fcJSON, width, height) {
            ...
            if (series) {
                ...
                if (!isPyramid && hasPoint && dataArr[0].displayValue) {
                    ...
                    point.labelWidht = smartLabel.getOriSize(smartTextObj.text).width;
    ...
    chartAPI('sparkline', {
        ...
        postSeriesAddition: function(hc, obj) {
            ...
            if ((length = index = hcDataArr && hcDataArr.length) > 0) {
                ...
                HCChartObj.openValue.label = HCChartObj.closeValue.label = HCChartObj.highLowValue.label =
                    HCChartObj.highLowValue.highLabel = HCChartObj.highLowValuelLowlabel = BLANKSTRING;
                if (hasValidValue) {
                    ...
                    if (pluckNumber(FCChartObj.showhighlowvalue, 1)) {
                        // Store complete high, low label to label for the space manager.
                        HCChartObj.highLowValue.label = '[' + highDisplayValue + ' | ' +
                            lowDisplayValue + ']';
                        // Store high, low label separately to render the labels
                        // Using different color style.
                        HCChartObj.highLowValue.highLabel = highDisplayValue;
                        HCChartObj.highLowValue.lowLabel = lowDisplayValue;
    ...
    realTimeExtension = {
        ...
        chart: function () {
            ...
            //RT Charts menu is not required but message logger is running
            //need to create a separate message logger menu
            //If user has opted not to show message logger menu from the beginning
            //do not add this method to callback. We will invoke the method
            //directly from message logger component on receipt of first log
                 ...
                 * @todo: Need to check for exceptions.
                 */
                if(optionsChart.messageGoesToLog) {
                    (hcObj.callbacks ||
            ...
            if (iapi.dataObj.alerts) {
                iapi.hcJSON.alerts = this.parseAlertObj(iapi.dataObj.alerts);
            }

            return hcObj;
        },

        parseAlertObj: function(alerts) {
            var iapi = this,
                numberFormatter = iapi.numberFormatter,
                i,
            ...
            for (i = 0; i < len; i += 1) {
                alrtObj = alertArr[i];
                minValue = numberFormatter.getCleanValue(alrtObj.minvalue);
                maxValue = numberFormatter.getCleanValue(alrtObj.maxvalue);
                //Parse the action
                action = pluck(alrtObj.action, '');
                //parse the param
                param = pluck(alrtObj.param, '');
                //In case of minValue, maxValue is not provided or action is not
                //defined, the alert object is meaningless and hence ignored.
                if(minValue === null || maxValue === null || action === ''){
                    continue;
                }

                //in case of minValue greater than the max value. Alter the range
                ...
                //If we have reach so far lets add a new object
                alrts.alert.push({
                    minvalue: minValue,
                    maxvalue: maxValue,
                    param: param,
            ...
            //return the new parsed alert object
            return alrts;
        },
...
//                                        angle : 90
    ...
    drawCylinder = (function () {
                ...
                for (key in hash) {
                    ...
                    if (attrList[key]) {
                        ...
                        if (shapeChanged) {
                            ...
                            if (animDuration) {
                                element.fluid.animate({
                                    path: blankArr.concat([M, x1, y2], [A, r, mathMax(r2, 1), 0, 0, 0, x2, y2],
                                        [L, x2, y3], [A, r, mathMax(r2, 1), 0, 0, 0, x1, y3, Z])
                                }, animDuration, ANIM_EFFECT);
        ...
        return function (x, y, r, h, r3dFactor, parentGroup, renderer, fluidHRatio,
            conColor, conAlpha, fluidColor, fluidAlpha, use3DLighting) {
            ...
            // 3D factor can't be zero for vml browsers
            if (R.vml) {
                r3dFactor = r3dFactor || 0.001;
            }
            ...
            //draw the back side
            if (use3DLighting) {
                colorStr = lightColor + COMMASTRING + darkColor + COMMASTRING + lightColor +
                    COMMASTRING + darkColor + COMMASTRING + darkColor1 + COMMASTRING +
                    darkColor1 + COMMASTRING + darkColor + COMMASTRING + lightColor;
            } else {
                colorStr = lightColor + COMMASTRING + darkColor + COMMASTRING + darkColor +
                    COMMASTRING + darkColor + COMMASTRING + darkColor + COMMASTRING +
                    darkColor + COMMASTRING + darkColor + COMMASTRING + lightColor;
            }
            cylinder.back = renderer.path(blankArr.concat([M, x1, y2], [A, r, r2, 0, 0, 0, x2, y2],
            ...
            //draw the fluid fill
            if (use3DLighting) {
                colorStr = fluidLightColor + COMMASTRING + fluidDarkColor;
            } else {
                colorStr = fluidDarkColor + COMMASTRING + fluidDarkColor;
            }
            cylinder.fluid = renderer.path(blankArr.concat([M, x1, y2], [A, r, mathMax(r2, 1), 0, 0, 0, x2, y2],
                [L, x2, y3], [A, r, mathMax(r2, 1), 0, 0, 0, x1, y3, Z]), cylinder)
            .attr({
                'stroke-width' : 0,
                fill : toRaphaelColor({
                    FCcolor : {
            ...
            .attr({
                'stroke-width' : 2,
                stroke : use3DLighting ? convertColor(fluidLightColor, fluidAlpha) : convertColor(fluidEdgeColor),
                fill : toRaphaelColor({
                    FCcolor : {
                        cx: 0.5,
                        cy: 0.7,
            ...
            if (use3DLighting) {
                ...
                colorStr = darkColor + COMMASTRING + darkColor + COMMASTRING + darkColor +
                    COMMASTRING + darkColor + COMMASTRING + darkColor + COMMASTRING +
                    darkColor + COMMASTRING + darkColor + COMMASTRING + darkColor;
            }
        ...
        drawWidgetValue: function (dataArr, animation) {
            ...
            for (; i < ln; i += 1) {
                ...
                if (!elements.pointers[i]) {
                    ...
                    if (topWidth) {
                        elements.pointers[i] = paper.path(pathComand, pGroup);
                    }
                    // Render using trianglepath
                    else {
                        elements.pointers[i] = paper.trianglepath(radius,
                            topWidthHF, -rearExtension, baseWidthHF,
                            -rearExtension, -baseWidthHF, 0, baseRadius,
                            baseRadius, pGroup);
                    }

                    point.graphic = graphic = elements.pointers[i];
        ...
        realtimeUpdate: function (updateObj, animation) {
            ...
            if (i) {
                while (i--) {
                    ...
                    valueDefined = false;
                    data = dataArr[i];
                    dialId = data.id && (data.id.toLowerCase && data.id.toLowerCase() || data.id);
                    // updateObj[dialId] value is used to support realtime update of the format &dialId=value
                    if ((values[i] !== undefined && values[i] !== '' && (valueDefined = true)) ||
                        (dialId && updateObj[dialId])) {
                        dataObj.value = pointObj.value = valueDefined ? values[i] : updateObj[dialId];
                        formatedValue = pointObj.displayvalue =
                            pointObj.tooltext = numberFormatter.dataLabels(pointObj.value);
                        pointObj.hasNewData = true;
                    }
    ...
    renderer('renderer.sparkline', {
        ...
        callbacks: [
            function() {
                ...
                smartLabel = renderer.smartLabel,
                chartOptions = options.chart,
                highValueLabel = chartOptions.highLowValue.highLabel,
                lowValueLabel = chartOptions.highLowValue.lowLabel,
                valuePadding = chartOptions.valuePadding,
                fixedPadding = 2,
